#!/usr/bin/env python3
"""
spaLLM QKV Multi-Seed Pipeline (Standalone Python Script)
Integrates joint spatial representation learning with QKV Within-Modality and Cross-Modality Attention:
...
# ===========================================================================
# --- COMBINED spaLLM QKV SOURCE CODE ---
import os
import random
import numpy as np
import pandas as pd
...
class QKVCrossFusionLayer(nn.Module):
    """
    QKV Cross-Fusion Layer supporting both 'local' (element-wise gated) 
    and 'global' (N x N scaled dot-product) cross-attention.
    """
...
class EncodingNetwork(Module):
    """Encoding network with modality-specific encoders, decoders, and QKVCrossFusionLayers."""
    def __init__(self, dim_in_omics1, dim_out_omics1, dim_in_omics2, dim_out_omics2, attention_type='local'):
        super().__init__()
        ...
        self.atten_feature1 = QKVCrossFusionLayer(dim_out_omics1, attention_type=attention_type)
        self.atten_feature2 = QKVCrossFusionLayer(dim_out_omics1, attention_type=attention_type)
        self.atten_feature = QKVCrossFusionLayer(dim_out_omics1, attention_type=attention_type)
        self.atten_omics2 = QKVCrossFusionLayer(dim_out_omics2, attention_type=attention_type)
        self.atten_cross = QKVCrossFusionLayer(dim_out_omics1, attention_type=attention_type)
...
def plot_spallm_visualizations(adata, title_prefix="spaLLM QKV", dname="dataset", seed=2024, save_dir=None):
    ...
    save_path = os.path.join(save_dir, f"spallm_qkv_plot_{dname}_seed_{seed}.png")
...
def plot_spallm_summary_boxplots(df_all, save_dir=None):
    ...
    for metric in metrics:
        ...
        ax.set_title(f"spaLLM QKV Model: {metric} Performance Across Datasets", fontsize=14, fontweight='bold')
        ...
        save_path = os.path.join(save_dir, f"spallm_qkv_boxplot_{metric.lower().replace('-', '_')}.png")
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
    ...
    plt.tight_layout()
    combined_path = os.path.join(save_dir, "spallm_qkv_boxplot_all_metrics.png")
...
class Train_spaLLM:
    ...
    def train(self, epochs=None):
        ...
        self.model = EncodingNetwork(self.dim_input1, self.dim_output, self.dim_input2, self.dim_output, attention_type=self.attention_type).to(self.device)
...
# ===========================================================================
# PIPELINE CODE SECTION FROM CELL 7
# ===========================================================================
# ==================================================================
# MAIN WORKFLOW RUNNER FOR spaLLM QKV
# ==================================================================
def run_spallm_qkv_workflow(dataset_name, dataset_cfg, env_mode, seed, device, show_plots=False):
    print(f"\n--- Running spaLLM QKV Seed: {seed} ---")
    fix_seed(seed)
    ...
    # 5. Tri-Algorithm Clustering (KMeans, Leiden, mclust) on spaLLM QKV Embedding
    ...
    # 5a. KMeans
    print(f"Running KMeans on spaLLM QKV embeddings with n_clusters={n_clusters}...")
    kmeans = KMeans(n_clusters=n_clusters, random_state=seed, n_init=10)
    adata.obs['kmeans'] = kmeans.fit_predict(spallm_emb).astype(str)

    # 5b. Leiden
    print("Running Leiden on spaLLM QKV embeddings...")
    ...
    # 5c. mclust
    print("Running mclust via rpy2 on spaLLM QKV embeddings...")
    ...
    metrics_kmeans = evaluate_clustering(
        ...
        adata.obs['kmeans'],
        spallm_emb,
        name=f"spaLLM QKV {dataset_name} Seed {seed} - KMeans"
    )
    ...
    metrics_leiden = evaluate_clustering(
        ...
        adata.obs['leiden'],
        spallm_emb,
        name=f"spaLLM QKV {dataset_name} Seed {seed} - Leiden"
    )
    ...
    metrics_mclust = evaluate_clustering(
        ...
        adata.obs['mclust'],
        spallm_emb,
        name=f"spaLLM QKV {dataset_name} Seed {seed} - mclust"
    )
    ...
    if show_plots:
        plot_spallm_visualizations(adata, title_prefix=f"spaLLM QKV {dataset_name} (Seed {seed})", dname=dataset_name, seed=seed)
        output_dir = '/kaggle/working' if os.path.exists('/kaggle/working') else '.'
        model.plot_loss(save_path=os.path.join(output_dir, f"spallm_qkv_loss_{dataset_name}_seed_{seed}.png"))
...
def main():
    ...
    for dname in datasets_to_run:
        ...
        print(f"\n=======================================================")
        print(f"STARTING SPA LLM QKV WORKFLOW: {dname.upper()}")
        print(f"=======================================================")
        
        for idx, seed in enumerate(SEEDS):
            show_plots = (idx == 0)
            try:
                results_list = run_spallm_qkv_workflow(dname, cfg, ENV_MODE, seed, device, show_plots=show_plots)
                if results_list:
    ...
    if len(all_results_flat) > 0:
        ...
        output_csv = os.path.join(output_dir, 'spallm_qkv_ablation_results.csv')
        df_all.to_csv(output_csv, index=False)
        print(f"All spaLLM QKV ablation study results saved to CSV at: {output_csv}")
